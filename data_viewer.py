...
class DataViewerWindow(BaseToplevelWindow):
    ...
    def __init__(self, master=None, callback_continuar: Optional[Callable] = None):
        """
    ...
    def _configurar_estilo_treeview(self):
        ...
        self.style.map("Custom.Treeview.Heading", background=[("active", "#4a4a4a")])
    ...
    def _crear_panel_tabla(self):
        ...
        scrollbar_x = ttk.Scrollbar(
            frame_tree, orient="horizontal", command=self.tree.xview
        )
    ...
    def _crear_panel_inferior(self):
        ...
        lbl_columna = ctk.CTkLabel(
            frame_seleccion, text="Seleccionar columna:", font=ctk.CTkFont(size=13)
        )
        ...
        frame_frecuencias = ctk.CTkScrollableFrame(
            frame_info, height=120, fg_color="transparent"
        )
    ...
    def _mostrar_datos_tabla(self):
        ...
        for col in columnas:
            ...
            max_width = max(
                len(str(col)) * 10, self.df[col].astype(str).str.len().max() * 10
            )
    ...
    def _on_columna_seleccionada(self, columna: str):
        ...
        try:
            ...
            messagebox.showerror("Error", f"Error al obtener frecuencias:\n{str(e)}")

    def _mostrar_frecuencias(self):
        """Muestra las frecuencias en el panel correspondiente."""
        try:
            if (
                self.frame_frecuencias is not None
                and self.frame_frecuencias.winfo_exists()
            ):
    ...
    def _continuar_analisis(self):
        ...
        if self.df is None:
            messagebox.showwarning(
                "Sin Datos", "Por favor, cargue un archivo antes de continuar."
            )
            return

        if not self.columna_seleccionada:
            messagebox.showwarning(
                "Sin Selección", "Por favor, seleccione una columna antes de continuar."
            )
            return
        ...
        if self.callback_continuar:
            self.callback_continuar(
                self.df, self.columna_seleccionada, n_registros, self.frecuencias
            )
        else:
            ParameterWindow(
                master=self,
    ...
    def _limpiar_datos(self):
        ...
        self.combo_columnas.configure(
            values=["-- Cargue un archivo primero --"], state="disabled"
        )
