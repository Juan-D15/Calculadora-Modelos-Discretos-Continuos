...
class AreaResultados:
    ...
    def limpiar(self):
        ...
        try:
            if (
                self.scrollable_frame is not None
                and self.scrollable_frame.winfo_exists()
            ):
    ...
    def _crear_seccion_estadisticas(self, row, datos):
        ...
        for label, valor in stats:
            ...
            lbl = ctk.CTkLabel(
                frame_stat, text=label, font=ctk.CTkFont(size=11), width=180, anchor="w"
            )
            lbl.pack(side="left")
    ...
    def _crear_seccion_forma(self, row, datos):
        ...
        if sesgo is not None:
            ...
            lbl_sesgo_val = ctk.CTkLabel(
                frame_sesgo, text=f"{sesgo:.4f}", font=ctk.CTkFont(size=11), anchor="w"
            )
            lbl_sesgo_val.pack(side="left", padx=5)

            color_sesgo = (
                "#e74c3c"
                if "Negativo" in interpretacion_sesgo
                else "#2ecc71"
                if "Positivo" in interpretacion_sesgo
                else "gray"
            )
        ...
        if curtosis is not None:
            ...
            color_curt = (
                "#e74c3c"
                if "Leptocúrtica" in interpretacion_curtosis
                else "#3498db"
                if "Platicúrtica" in interpretacion_curtosis
                else "gray"
            )
